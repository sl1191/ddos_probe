...
use std::io;
use crate::flow_analyzer::metrics::Metrics;  // 引入指标计数器（PPS/QPS/BPS）

/// 抓包器统一接口（多实现适配：开发用pcap/生产用AF_XDP/测试用Dummy）
pub trait Capture {
    /// 创建抓包器实例
    /// - 参数：`iface` - 网卡接口名（如"eth0"）
    /// - 返回：成功时返回抓包器实例，失败时返回IO错误
    fn new(iface: &str) -> io::Result<Self> where Self: Sized;

    /// 批量捕获数据包并更新指标
    /// - 参数：`metrics` - 全局指标计数器（原子操作安全）
    /// - 返回：成功时返回捕获的数据包数量，失败时返回IO错误
...
pub mod pcap {
    use super::*;
    // 明确引用外部 crate 的 Capture 和 Error
    ...
    impl super::Capture for PcapCapture {
        /** 创建pcap抓包器（默认配置：混杂模式+非阻塞+MTU=1500字节） */
        fn new(iface: &str) -> io::Result<Self> {
            ...
            Ok(Self { driver: capture }) // 使用具体的 Capture<Active> 类型
        }
...
pub mod af_xdp {
    use super::*;
    use afxdp::socket::{Socket, Config};  // AF_XDP用户态socket及配置
    ...
    impl Capture for AfXdpCapture {
        ...
        fn new(iface: &str) -> io::Result<Self> {
            ...
            Ok(Self { socket, buffer: vec![0; 1500] })
        }
